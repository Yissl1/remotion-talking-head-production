...
export const TechCard:React.FC<{kicker:string;title:string;detail?:string;color?:string;accent?:string;boxed?:boolean;edgeGlow?:boolean;rect:SafeRect;align?:'left'|'right'} & TimedProps>=({kicker,title,detail,color=WHITE,accent=BLUE,boxed=false,edgeGlow=false,rect,align='left',startFrame,endFrame})=>{
  ...
  return <div style={{position:'absolute',...rect,zIndex:90,color:WHITE,textAlign:align,...group}}>
    {boxed&&<div style={{position:'absolute',inset:-12,background:'rgba(12,14,17,.92)',border:`1px solid ${accent}`,borderRadius:8,boxShadow:edgeGlow?`0 0 12px ${accent}55`:'0 10px 28px rgba(0,0,0,.28)'}}/>}
...
export const LineIconBadge:React.FC<{
  ...
  return <div style={{position:'absolute',...rect,zIndex:95,display:'flex',flexDirection:reverse?'row-reverse':'row',alignItems:'center',gap:micro?9:18,color:WHITE,opacity,transform:`translateX(${(reverse?1:-1)*24*(1-enter)}px) scale(${.96+.04*enter})`}}>
    {boxed&&<div style={{position:'absolute',inset:-10,borderRadius:radius,background:'rgba(17,19,21,.78)',border:`1px solid ${accent}88`,boxShadow:edgeGlow?`0 0 18px ${accent}44,0 12px 32px rgba(0,0,0,.28)`:'0 12px 32px rgba(0,0,0,.24)',pointerEvents:'none'}}/>}
    <div style={{position:'relative',width:circle,height:circle,flex:'0 0 auto',borderRadius:'50%',display:'grid',placeItems:'center',color:accent,border:`2px solid ${accent}`,background:'rgba(12,14,17,.72)',boxShadow:edgeGlow?`0 0 14px ${accent}55`:'0 0 10px rgba(0,0,0,.18)'}}><IconGlyph name={icon} size={micro?26:(compact?39:50)}/></div>
...
export const ProcessRail:React.FC<{
  steps:{label:string;icon:IconName;accent?:string}[];rect:SafeRect;
} & TimedProps>=({steps,rect,startFrame,endFrame})=>{
  const frame=useCurrentFrame();
  if(frame<startFrame||frame>=endFrame)return null;
  return <div style={{position:'absolute',...rect,zIndex:95,display:'flex',alignItems:'center',justifyContent:'space-between',color:WHITE}}>
    {steps.map((step,index)=>{
      ...
      return <React.Fragment key={`${step.label}-${index}`}>
        {index>0&&<div style={{height:2,flex:1,maxWidth:68,margin:'0 10px',background:'rgba(247,248,250,.16)',overflow:'hidden'}}><div style={{height:'100%',width:`${line*100}%`,background:accent}}/></div>}
        <div style={{width:132,textAlign:'center',...motion}}>
          <div style={{width:70,height:70,margin:'0 auto',borderRadius:'50%',display:'grid',placeItems:'center',color:accent,border:`2px solid ${accent}`,background:'rgba(12,14,17,.78)',boxShadow:`0 0 14px ${accent}33`}}><IconGlyph name={step.icon} size={36}/></div>
          <div style={{marginTop:10,fontSize:20,fontWeight:950,lineHeight:1.1}}>{step.label}</div>
        </div>
      </React.Fragment>;
    })}
  </div>;
...
export const SpeakerBubble:React.FC<{src:string;rect?:SafeRect;color?:string;edgeGlow?:boolean;objectPosition?:string;sourceStartFrame?:number} & TimedProps>=({src,rect={left:70,top:766,width:250,height:250},color=BLUE,edgeGlow=false,objectPosition='50% 42%',startFrame,endFrame,sourceStartFrame=startFrame})=>{
  const frame=useCurrentFrame();
  if(frame<startFrame||frame>=endFrame)return null;
...
export const MetricCard:React.FC<{value:number;prefix?:string;suffix?:string;decimals?:number;label?:string;rect:SafeRect;boxed?:boolean;edgeGlow?:boolean} & TimedProps>=({value,prefix='',suffix='',decimals=0,label,rect,boxed=false,edgeGlow=false,startFrame,endFrame})=>{
  ...
  const current=value === 0 ? 0 : value*(0.12+0.88*eased);
...
export const IconLabelChip:React.FC<{icon:IconName;label:string;accent?:string;rect:SafeRect;boxed?:boolean;edgeGlow?:boolean;detail?:string} & TimedProps>=({icon,label,accent=BLUE,rect,boxed=false,edgeGlow=false,detail,startFrame,endFrame})=>{
  ...
  return <div style={{position:'absolute',...rect,zIndex:95,display:'flex',alignItems:'center',gap:10,padding:boxed?'8px 12px':'0',borderRadius:boxed?10:0,background:boxed?'rgba(12,14,17,.78)':'transparent',border:boxed?`1px solid ${accent}88`:'none',boxShadow:edgeGlow?`0 0 16px ${accent}44`:'none',color:WHITE,...group}}>
    <div style={{width:34,height:34,borderRadius:'50%',display:'grid',placeItems:'center',color:accent,border:`1px solid ${accent}`,background:'rgba(12,14,17,.72)',flex:'0 0 auto'}}><IconGlyph name={icon} size={22}/></div>
...
export const BilingualSubtitle: React.FC<{
  ...
  return <div style={{position: 'absolute', left: 240, right: 240, bottom: 43, zIndex: 140, display: 'flex', justifyContent: 'center', pointerEvents: 'none', opacity: enter * exit, transform: `translateY(${(1 - enter) * 6}px)`}}>
    <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, maxWidth: 1400}}>
      <div style={{display: 'inline-block', maxWidth: 1320, padding: '5px 16px 7px', background: 'rgba(0,0,0,.64)', color: WHITE, fontSize: 42, lineHeight: 1.06, fontWeight: 950, textAlign: 'center', letterSpacing: '.01em', whiteSpace: 'pre-line', textShadow: '0 2px 4px rgba(0,0,0,.9)'}}>{text}</div>
      {english && <div style={{display: 'inline-block', maxWidth: 1100, padding: '4px 14px 5px', background: 'rgba(0,0,0,.64)', color: WHITE, fontSize: 28, lineHeight: 1.05, fontWeight: 800, textAlign: 'center', letterSpacing: '.015em', whiteSpace: 'pre-line', textShadow: '0 2px 4px rgba(0,0,0,.9)'}}>{english}</div>}
    </div>
  </div>;
};
