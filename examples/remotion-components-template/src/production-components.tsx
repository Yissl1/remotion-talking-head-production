...
type IconName='ai'|'cut'|'mic'|'layers'|'flow'|'compare'|'warning'|'spark';
...
export const DataBars:React.FC<{
  ...
  return <div style={{position:'absolute',...rect,zIndex:95,color:WHITE,textAlign:align}}>
    ...
    <div style={{marginTop:18,display:'grid',gap:15}}>{items.map((item,index)=>{
      ...
      return <div key={`${item.label}-${index}`} style={{opacity:out,...enterExit(frame,reveal,endFrame)}}>
...
export const SpeakerBubble:React.FC<{src:string;rect?:SafeRect;color?:string;edgeGlow?:boolean;objectPosition?:string;sourceStartFrame?:number} & TimedProps>=({src,rect={left:70,top:766,width:250,height:250},color=BLUE,edgeGlow=false,objectPosition='50% 42%',sourceStartFrame=0,startFrame,endFrame})=>{
  const frame=useCurrentFrame();
  if(frame<startFrame||frame>=endFrame)return null;
